...
`/vagrant/projects`). Apache loads its virtual hosts at runtime from the mounted
`config/apache/`, so `<path>` is a clone of the GitHub repository below; its
README's "Virtual hosts" section says which confs ship and how to add one for a
project.
