...
MYSQL_VERSION="${MYSQL_VERSION:-$(mysqld --version | awk '{print $3}')}"
...
get_mysql_user() {
	export MYUSER="${USER:-mysql}"
	export MYGROUP="$MYUSER"

	# MYSQLD_OPTS contiene gli argomenti passati a mysqld dal servizio systemd
	if [ -n "${MYSQLD_OPTS:-}" ]; then
		# parsing shell-like per gestire correttamente anche eventuali quote
		eval "mysqld_opts=( $MYSQLD_OPTS )"

		for ((i = 0; i < ${#mysqld_opts[@]}; i++)); do
			case "${mysqld_opts[i]}" in
			--user=*)
				MYUSER="${mysqld_opts[i]#--user=}"
				MYGROUP="$MYUSER"
				break
				;;
			--user | -u)
				((++i < ${#mysqld_opts[@]})) || break
				MYUSER="${mysqld_opts[i]}"
				MYGROUP="$MYUSER"
				break
				;;
			esac
...
set_mysql_permissions() {
	...
	install /dev/null -m0640 -o"$MYUSER" -g"$MYGROUP" "/var/log/mysqld.log"

	# Create data directories and fix permissions
	install -d -m0751 -o"$MYUSER" -g"$MYGROUP" /var/lib/mysql
	chown -R "$MYUSER":"$MYGROUP" /var/lib/mysql

	# Fix permissions for socket and pid files
	chown -R "$MYUSER":"$MYGROUP" /var/run/mysqld
}
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ "$MYUSER" = 'root' ]; then
			...
			mysql_note "Switching to dedicated user '${MYUSER}'"
			exec runuser -u "$MYUSER" -- "$BASH_SOURCE" "$@"
		fi
...
# if command is empty or starts with an option, prepend mysqld
if [ -z "${1:-}" ] || [ "${1:0:1}" = '-' ]; then
	set -- mysqld "$@"
fi
